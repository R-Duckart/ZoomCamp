...
select
    ...
    cast('Yellow' as {{ dbt.type_string() }}) as service_type,
    cast(payment_type as {{ dbt.type_int() }}) as payment_type,
    cast(1 as {{ dbt.type_int() }}) as trip_type,

    -- timestamps
    cast(tpep_pickup_datetime as {{ dbt.type_timestamp() }}) as pickup_datetime,
    cast(tpep_dropoff_datetime as {{ dbt.type_timestamp() }}) as dropoff_datetime,
    ...
    cast(null as {{ dbt.type_float() }}) as ehail_fee,
...
qualify row_number() over (
    partition by VendorID, tpep_pickup_datetime, tpep_dropoff_datetime, PULocationID, DOLocationID, RatecodeID
